...
def make_tweet_fn(text, time, lat, lon):
    ...
    """
    "*** YOUR CODE HERE ***"
    tw = {'text': text, 'time': time, 'lat': lat, 'lon': lon}

    def tweet_fn(arg):
        assert arg == 'text' or arg == 'time' or arg == 'lat' or arg == 'lon'
        return tw[arg]
    return tweet_fn
...
def extract_words(text):
    ...
    """
    "*** YOUR CODE HERE ***"
    clean_word_list = ''.join([c if c in ascii_letters else ' ' for c in text]).split(' ')
    return [w for w in clean_word_list if w]
...
def analyze_tweet_sentiment(tweet):
    ...
    """
    "*** YOUR CODE HERE ***"
    total_sentiment_value, word_cnt = 0, 0
    word_list = tweet_words(tweet)
    for word in word_list:
        word_sentiment = get_word_sentiment(word)
        if has_sentiment(word_sentiment):
            total_sentiment_value += sentiment_value(word_sentiment)
            word_cnt += 1
    return make_sentiment(total_sentiment_value / word_cnt) if word_cnt else make_sentiment(None)
...
def find_centroid(polygon):
    ...
    area, centroid_lat, centroid_lon = 0, 0, 0

    for i in range(len(polygon) - 1):
        lat1, lat2 = latitude(polygon[i]), latitude(polygon[i + 1])
        lon1, lon2 = longitude(polygon[i]), longitude(polygon[i + 1])
        tail = lat1 * lon2 - lat2 * lon1
        area += 0.5 * tail
        centroid_lat += (lat1 + lat2) * tail
        centroid_lon += (lon1 + lon2) * tail
...
def find_state_center(polygons):
    ...
    cxa_sum, cya_sum, area_sum = 0, 0, 0
    for polygon in polygons:
        polygon_lat, polygon_lon, polygon_area = find_centroid(polygon)
        cxa_sum += polygon_lat * polygon_area
        cya_sum += polygon_lon * polygon_area
        area_sum += polygon_area
    return make_position(cxa_sum / area_sum, cya_sum / area_sum)
...
def group_tweets_by_state(tweets):
    ...
    tweets_by_state = {}
    "*** YOUR CODE HERE ***"
    # populate state centers and their centroids
    state_centers = {}
    for state in us_states:
        state_centers[state] = find_state_center(us_states[state])

    for tweet in tweets:
        closest_geo_distance, closest_state = 0, ''
        location = tweet_location(tweet)
        position = make_position(latitude(location), longitude(location))
        for state in state_centers:
            distance = geo_distance(position, state_centers[state])
            if not closest_geo_distance or distance < closest_geo_distance:
                closest_geo_distance = distance
                closest_state = state
        if closest_state not in tweets_by_state.keys():
...
def average_sentiments(tweets_by_state):
    ...
    for state in tweets_by_state:
        s_sum, s_count = 0, 0
        tweet_list = tweets_by_state[state]
        for tweet in tweet_list:
            s = analyze_tweet_sentiment(tweet)
            if has_sentiment(s):
                s_sum += sentiment_value(s)
                s_count += 1
        if s_count:
            averaged_state_sentiments[state] = s_sum / s_count
